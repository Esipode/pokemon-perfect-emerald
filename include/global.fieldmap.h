...
struct PlayerAvatar
{
    ...
    /*0x0A*/ u8 bikeFrameCounter; // acts as a timer for the bike's trick states (turn delay, wheelie hold).
    /*0x0B*/ u8 bikeSpeed;
    /*0x0C*/ u8 bikeSpeedCounter; // indexes the bike's speed callbacks while moving; 0 is the slowest.
    /*0x10*/ u32 directionHistory; // up/down/left/right history is stored in each nybble, but using the field directions and not the io inputs.
    /*0x14*/ u32 abStartSelectHistory; // same as above but for A + B + start + select only
    // these two are timer history arrays which [0] is the active timer for the trick input check. every element is backed up to the next element upon update.
    /*0x18*/ u8 dirTimerHistory[8];
    /*0x20*/ u8 abStartSelectTimerHistory[8];
    u16 lastSpinTile;
};
