...
#define B_FLAG_AI_VS_AI_BATTLE      FLAG_AI_BATTLES // If this flag is set, the player's Pokémon will be controlled by the AI in the next battle.
...
#define B_FLAG_NO_WHITEOUT          FLAG_EMPORIUM_NO_WHITEOUT // If this flag is set, the player can not white out against Trainers. Please note that the party is not healed automatically! Owned by the Battle Emporium (armed/cleared around its challenger battles); nothing else sets it.
...
#define B_HP_PERCENTAGE_DISPLAY             FALSE      // New-game default for the FOE HP option (option_menu.c): TRUE makes it "BAR + %" instead of "BAR". Players can override it per side in the options menu; this only seeds SetDefaultOptions() and the fallback for pre-option saves.
...
#define B_ENCOUNTER_SCRIPTING           TRUE       // Enables scripted battle encounters. See docs.
