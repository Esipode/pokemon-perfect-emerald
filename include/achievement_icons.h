...
#define ACHIEVEMENT_POINTS_ICON_SIZE 16
...
#define ACHIEVEMENT_POINTS_ICON_Y(textY) ((textY) - 1)

// Call once per menu init, after that menu has loaded its own text palette
// into bgPaletteNum -- this appends the icon's colours to that palette's
// unused high entries, so it must not be undone by a later LoadPalette over
// the same slots. See src/achievement_icons.c for why the icon can't just be
// blitted with its own palette.
void AchievementIcons_LoadPointsIcon(u8 bgPaletteNum);

// Draws the points icon into a window at window-local pixel coordinates,
// replacing the word "Points"/"pts" next to a point value. Cleared by the
// same FillWindowPixelBuffer that clears the window's text.
