...
#define FOREACH_DEX_VARIANT_FLAG_SLOT(F) \
    DEX_VARIANT_FORM_IF(P_ALOLAN_FORMS, \
        ...
        F(VULPIX_ALOLA) F(NINETALES_ALOLA) F(DIGLETT_ALOLA) F(DUGTRIO_ALOLA) F(MEOWTH_ALOLA) F(PERSIAN_ALOLA)) \
    DEX_VARIANT_FORM_IF(P_GALARIAN_FORMS, F(MEOWTH_GALAR)) \
    DEX_VARIANT_FORM_IF(P_HISUIAN_FORMS, F(GROWLITHE_HISUI) F(ARCANINE_HISUI)) \
    DEX_VARIANT_FORM_IF(P_ALOLAN_FORMS, F(GEODUDE_ALOLA) F(GRAVELER_ALOLA) F(GOLEM_ALOLA)) \
    DEX_VARIANT_FORM_IF(P_GALARIAN_FORMS, \
        F(PONYTA_GALAR) F(RAPIDASH_GALAR) F(SLOWPOKE_GALAR) F(SLOWBRO_GALAR) F(SLOWKING_GALAR) F(FARFETCHD_GALAR)) \
    DEX_VARIANT_FORM_IF(P_ALOLAN_FORMS, F(GRIMER_ALOLA) F(MUK_ALOLA)) \
    DEX_VARIANT_FORM_IF(P_HISUIAN_FORMS, F(VOLTORB_HISUI) F(ELECTRODE_HISUI)) \
    DEX_VARIANT_FORM_IF(P_ALOLAN_FORMS, F(EXEGGUTOR_ALOLA) F(MAROWAK_ALOLA)) \
    DEX_VARIANT_FORM_IF(P_GALARIAN_FORMS, F(WEEZING_GALAR) F(MR_MIME_GALAR)) \
    DEX_VARIANT_FORM_IF(P_PALDEAN_FORMS, F(TAUROS_PALDEA_COMBAT) F(TAUROS_PALDEA_BLAZE) F(TAUROS_PALDEA_AQUA)) \
    DEX_VARIANT_FORM_IF(P_GALARIAN_FORMS, F(ARTICUNO_GALAR) F(ZAPDOS_GALAR) F(MOLTRES_GALAR)) \
    DEX_VARIANT_FORM_IF(P_HISUIAN_FORMS, F(TYPHLOSION_HISUI)) \
    DEX_VARIANT_FORM_IF(P_PALDEAN_FORMS, F(WOOPER_PALDEA)) \
    DEX_VARIANT_FORM_IF(P_HISUIAN_FORMS, F(QWILFISH_HISUI) F(SNEASEL_HISUI)) \
    DEX_VARIANT_FORM_IF(P_GALARIAN_FORMS, F(CORSOLA_GALAR) F(ZIGZAGOON_GALAR) F(LINOONE_GALAR)) \
    DEX_VARIANT_FORM_IF(P_HISUIAN_FORMS, F(SAMUROTT_HISUI) F(LILLIGANT_HISUI)) \
    DEX_VARIANT_FORM_IF(P_GALARIAN_FORMS, F(DARUMAKA_GALAR) F(DARMANITAN_GALAR_STANDARD) F(YAMASK_GALAR)) \
    DEX_VARIANT_FORM_IF(P_HISUIAN_FORMS, F(ZORUA_HISUI) F(ZOROARK_HISUI)) \
    DEX_VARIANT_FORM_IF(P_GALARIAN_FORMS, F(STUNFISK_GALAR)) \
    DEX_VARIANT_FORM_IF(P_HISUIAN_FORMS, \
        F(BRAVIARY_HISUI) F(SLIGGOO_HISUI) F(GOODRA_HISUI) F(AVALUGG_HISUI) F(DECIDUEYE_HISUI))

enum DexVariantFlagSlot
{
    #define DEX_VARIANT_SLOT_ENUM(name) DEX_VARIANT_SLOT_ ##name,
    FOREACH_DEX_VARIANT_FLAG_SLOT(DEX_VARIANT_SLOT_ENUM)
    #undef DEX_VARIANT_SLOT_ENUM
    DEX_VARIANT_FLAG_SLOT_COUNT
};

#define DEX_FLAG_SLOT_COUNT (NATIONAL_DEX_COUNT + DEX_VARIANT_FLAG_SLOT_COUNT)
