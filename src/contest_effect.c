// The contest minigame was removed (RAM reclamation, stage 3c). gContestEffects (in
// data/contest_moves.h) is still read by non-contest move-info UI (Pokedex, Move Relearner)
// for each move's appeal/jam/description, and its per-entry .function pointers -- which used
// to run the effect during an actual contest turn -- are kept pointing at these functions so
// that data file doesn't need editing, but nothing ever calls through them anymore, so every
// body below is a no-op.
#include "global.h"
#include "move.h"
#include "random.h"
