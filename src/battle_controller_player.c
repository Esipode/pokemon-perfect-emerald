...
static void PlayerHandleEndLinkBattle(enum BattlerId battler)
{
    ...
    gSaveBlock2Ptr->disableRecordBattle = gBattleResources->bufferA[battler][2];
    FadeOutMapMusic(5);
...
static bool32 ShouldShowTypeEffectiveness(u32 targetId)
{
    ...
    if (B_SHOW_EFFECTIVENESS == SHOW_EFFECTIVENESS_CAUGHT)
        return GetSetPokedexFlagBySpecies(gBattleMons[targetId].species, FLAG_GET_CAUGHT);

    if (B_SHOW_EFFECTIVENESS == SHOW_EFFECTIVENESS_SEEN)
        return GetSetPokedexFlagBySpecies(gBattleMons[targetId].species, FLAG_GET_SEEN);
...
static void MoveSelectionDisplayMoveEffectiveness(u32 foeEffectiveness, enum BattlerId battler)
{
    ...
    static const u8 superEffectiveIcon[] =  _("{COLOR CUSTOM_GREEN}{UP_ARROW}");
    static const u8 notVeryEffectiveIcon[] =  _("{COLOR CUSTOM_RED}{DOWN_ARROW}");
    static const u8 mostlyIneffectiveIcon[] =  _("{COLOR CUSTOM_RED}{TRIANGLE_UPSIDE_DOWN}");
    static const u8 immuneIcon[] =  _("{COLOR DYNAMIC_COLOR6}{BIG_MULT_X}");
    struct ChooseMoveStruct *moveInfo = (struct ChooseMoveStruct *)(&gBattleResources->bufferA[battler][4]);
    u8 *txtPtr;
