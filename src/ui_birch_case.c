    ...
    if (FlagGet(FLAG_RANDOMIZE_MON))
    {
        ...
        for (setIndex = 0; setIndex < 3; setIndex++)
        {
            for (slotIndex = 0; slotIndex < 9; slotIndex++)
            {
                ...
                {
                    sCurrentChoices[setIndex][slotIndex].species = GetRandomSpecies(setIndex, slotIndex);
                }
            }
        }
    }
...
static void BirchCase_GiveMon() // Function that calls the GiveMon function pulled from Expansion by Lunos and Ghoulslash
{
    ...
    gSpecialVar_Result = BirchCase_GiveMonParameterized(sCurrentChoices[gSpecialVar_Result][sBirchCaseDataPtr->handPosition].species, sCurrentChoices[gSpecialVar_Result][sBirchCaseDataPtr->handPosition].level, \
                ...
                sCurrentChoices[gSpecialVar_Result][sBirchCaseDataPtr->handPosition].nature, sCurrentChoices[gSpecialVar_Result][sBirchCaseDataPtr->handPosition].abilityNum, \
