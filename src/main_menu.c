 ...
 * Task_NewGameBirchSpeech_StartBirchMonPlatformFade
 * Task_NewGameBirchSpeech_StartBirchMonPlatformFade
 ...
 * Task_NewGameBirchSpeech_ReshowBirchMon
 ...
 *  - Advances to Task_NewGameBirchSpeechSub_WaitForMon
 * Task_NewGameBirchSpeechSub_WaitForMon
...
static void Task_NewGameBirchSpeechSub_WaitForMon(u8);
static void Task_NewGameBirchSpeech_StartBirchMonPlatformFade(u8);
...
static void Task_NewGameBirchSpeech_ReshowBirchMon(u8);
...
{
    u8 spriteId = gTasks[sBirchSpeechMainTaskId].tMonSpriteId;
    ...
    gTasks[taskId].func = Task_NewGameBirchSpeechSub_WaitForMon;
    gTasks[sBirchSpeechMainTaskId].tTimer = 0;
}

static void Task_NewGameBirchSpeechSub_WaitForMon(u8 taskId)
{
    s16 *data = gTasks[taskId].data;
    struct Sprite *sprite = &gSprites[gTasks[sBirchSpeechMainTaskId].tMonSpriteId];
...
static void Task_NewGameBirchSpeech_AndYouAre(u8 taskId)
{
    if (!RunTextPrintersAndIsPrinter0Active())
    {
        ...
        AddTextPrinterForMessage(TRUE);
        gTasks[taskId].func = Task_NewGameBirchSpeech_StartBirchMonPlatformFade;
    }
}

static void Task_NewGameBirchSpeech_StartBirchMonPlatformFade(u8 taskId)
{
    if (!RunTextPrintersAndIsPrinter0Active())
    {
...
static void Task_NewGameBirchSpeech_SlidePlatformAway2(u8 taskId)
{
    ...
    {
        gTasks[taskId].func = Task_NewGameBirchSpeech_ReshowBirchMon;
    }
}

static void Task_NewGameBirchSpeech_ReshowBirchMon(u8 taskId)
{
    u8 spriteId;
...
{
    // Kept across the naming screen / player color returns so the same mon shows all sequence.
    if (sBirchSpeechMonSpecies == SPECIES_NONE)
...
static void AddBirchSpeechObjects(u8 taskId)
{
    ...
    monSpriteId = NewGameBirchSpeech_CreateRandomMonSprite(100, 0x4B);
